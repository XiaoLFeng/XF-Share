...
if (!empty($_COOKIE['user'])/*检查COOKIE*/) {
    // 删除数据
    $delete_url = $_SERVER['HTTP_HOST'].'/api/article/delete/?ssid='.xfs_ssid().'&id='.$article_id;    
    ...
    if ($delete['output'] == 'SUCCESS') {
        echo <<<EOF
                ...
                EOF;
    } elseif ($delete['output'] == 'ID_ARTICLE_ERROR') {
        echo <<<EOF
            <script language="javascript">
                alert( "删除失败，数据库删除错误！" )
                window.history.go(-1);
            ...
            EOF;
    } elseif ($delete['output'] == 'ID_ARTICLE_NONE') {
        echo <<<EOF
            <script language="javascript">
                alert( "参数 Query[id] 错误，没有该文章" )
                window.history.go(-1);
            </script>
            EOF;
    } elseif ($delete['output'] == 'ID_NONE') {
        echo <<<EOF
            <script language="javascript">
                alert( "参数 Query[id] 缺失" )
                window.history.go(-1);
            </script>
            EOF;
